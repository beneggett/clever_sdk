...
class CleverSDK::Api::CoursesTest < CleverSDK::Test
  def test_courses
    VCR.use_cassette("api/courses") do
  ...
  def test_courses_with_limit
    VCR.use_cassette("api/courses_with_limit") do
  ...
  def test_courses_with_starting_after
    VCR.use_cassette("api/courses_with_starting_after") do
  ...
  def test_courses_next
    VCR.use_cassette("api/courses_next") do
      courses = CleverSDK.client("TEST_TOKEN").courses
