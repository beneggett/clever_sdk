...
class CleverApi::Client::EventsTest < CleverApi::Test
  def test_events
    VCR.use_cassette("events") do
      events = CleverApi.client("TEST_TOKEN").events

      assert_kind_of CleverApi::Data::Events, events
  ...
  def test_events_with_limit
    VCR.use_cassette("events_with_limit") do
      events = CleverApi.client("TEST_TOKEN").events(limit: 25)
  ...
  def test_events_with_starting_after
    VCR.use_cassette("events_with_starting_after") do
      events = CleverApi.client("TEST_TOKEN").events(starting_after: "640df950d306111b2385f72b")
      ...
      assert_kind_of DateTime, event.created
      assert_kind_of CleverApi::Data::User, event.object
  ...
  def test_events_next
    VCR.use_cassette("events_next") do
      ...
      assert_kind_of CleverApi::Data::Events, next_events
      assert_equal 100, next_events.count
      assert next_events.next?

      next_event = next_events.first

      assert_kind_of CleverApi::Data::Event, next_event
      ...
      assert_kind_of CleverApi::Data::User, next_event.object
