...
module CleverApi
  class SSO
    ...
    def initialize(code, configuration = CleverApi.configuration)
    ...
    def user_details_from_access_token(access_token)
      me_response = CleverApi.authentication(configuration).me(access_token)
      user_id = me_response.id

      CleverApi.client(access_token).user(user_id)
    end
  end
end
